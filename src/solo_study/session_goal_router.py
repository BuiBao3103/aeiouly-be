...
async def create_session_goal(
    ...
    """
    try:
        return await service.create_goal(goal_data, current_user.id, db)
...
async def get_session_goals(
    ...
    """
    try:
        return await service.get_goals(current_user.id, db, pagination, status)
    except SessionGoalValidationException as e:
        raise session_goal_validation_exception(str(e))
    except Exception as e:
...
async def get_session_goal(
    ...
    """
    try:
        return await service.get_goal_by_id(goal_id, current_user.id, db)
...
async def update_session_goal(
    ...
    """
    try:
        return await service.update_goal(goal_id, goal_data, current_user.id, db)
    except SessionGoalNotFoundException as e:
        raise session_goal_not_found_exception(goal_id)
    except SessionGoalValidationException as e:
        raise session_goal_validation_exception(str(e))
...
async def delete_session_goal(
    ...
    """
    try:
        success = await service.delete_goal(goal_id, current_user.id, db)
        if success:
