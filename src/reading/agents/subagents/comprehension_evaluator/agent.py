...
This agent evaluates comprehension for parallel processing in English summaries.
"""

from google.adk.agents import LlmAgent
from pydantic import BaseModel, Field

class ComprehensionEvaluationRequest(BaseModel):
    """Request schema for comprehension evaluation"""
...
class ComprehensionEvaluationResult(BaseModel):
    """Response schema for comprehension evaluation"""
    score: int = Field(..., ge=0, le=100, description="Comprehension score 0-100")
    feedback: str = Field(..., description="Comprehension feedback and suggestions")

comprehension_evaluator_agent = LlmAgent(
    name="comprehension_evaluator_agent",
    model="gemini-2.0-flash",
    description="Evaluates comprehension for parallel processing",
    instruction="""
    Bạn là AI chuyên đánh giá khả năng hiểu nội dung (comprehension) của người đọc tiếng Anh.
    
    NHIỆM VỤ:
    - Đánh giá mức độ hiểu nội dung bài đọc qua bài tóm tắt
    ...
    - Trả về JSON format
    """,
    output_schema=ComprehensionEvaluationResult,
    output_key="comprehension_evaluation_result",
