...
class WritingService:
    ...
    async def create_writing_session(
        ...
        try:
            ...
            content = types.Content(
                role="user", 
                parts=[types.Part(text=f"Generate Vietnamese text for topic: {session_data.topic}, difficulty: {session_data.difficulty.value}, sentences: {session_data.total_sentences}")]
            )
            ...
            try:
                ...
                # If no text generated, create a fallback
                if not generated_text:
                    print("No text generated by agent, using fallback")
                    generated_text = self._generate_fallback_text(session_data.topic, session_data.difficulty, session_data.total_sentences)
                    # For fallback, create simple sentences array
                ...
                import traceback
                traceback.print_exc()
                # Use fallback text if agent fails
                generated_text = self._generate_fallback_text(session_data.topic, session_data.difficulty, session_data.total_sentences)
                # For fallback, create simple sentences array
                vietnamese_sentences = [generated_text]
    ...
    def _generate_fallback_text(self, topic: str, difficulty: CEFRLevel, total_sentences: int) -> str:
        """Generate fallback Vietnamese text when agent fails"""
        # Simple fallback text based on topic and difficulty
        fallback_texts = {
            "school": [
                "Tôi đi học mỗi ngày.",
                "Trường học có nhiều học sinh.",
                "Giáo viên dạy chúng tôi bài học.",
                "Chúng tôi học toán và tiếng Anh.",
                "Tôi thích đi học với bạn bè."
            ],
            "family": [
            ...
            "food": [
                ...
                "Bánh mì là món ăn sáng phổ biến."
            ]
        }
        
        # Get sentences for topic or use generic ones
        sentences = fallback_texts.get(topic.lower(), [
            "Đây là câu đầu tiên.",
            "Đây là câu thứ hai.",
            "Đây là câu thứ ba.",
            "Đây là câu thứ tư.",
        ...
        if difficulty in [CEFRLevel.A1, CEFRLevel.A2]:
            # Keep simple sentences
            pass
        elif difficulty in [CEFRLevel.B1, CEFRLevel.B2]:
            # Add some complexity
            sentences = [s.replace(".", " và tôi rất thích.") for s in sentences]
        else:
            # More complex structures
            sentences = [f"Mặc dù có nhiều khó khăn, {s.lower()}" for s in sentences]
        
        # Return requested number of sentences
        return ".".join(sentences[:total_sentences]) + "."
