...
class WritingService:
    ...
    async def create_writing_session(
        ...
        try:
            ...
            db_session.current_sentence = self._get_sentence_by_index(sentences, 0)
            db.commit()
    ...
    async def get_writing_session(self, session_id: int, user_id: int, db: Session) -> Optional[WritingSessionResponse]:
        ...
        try:
            ...
            if isinstance(vietnamese_sentences_data, dict):
                ...
                # Fallback: try old format (separate sentences and full_text keys)
                vietnamese_sentences = agent_session.state.get("sentences", [])
...
Độ khó (CEFR level): {level}"""
    ...
    def _get_sentences_from_db(self, session: WritingSession) -> List[str]:
        """Get sentences array from database session, parsing if needed"""
        # Try to get from agent state first
        try:
            # This is called during bootstrap, so we'll parse from DB
            if session.vietnamese_text:
                # Simple parsing: split by sentence endings
                import re
                sentences = re.split(r'[.!?]+', session.vietnamese_text)
                sentences = [s.strip() for s in sentences if s.strip()]
                return sentences if sentences else [session.vietnamese_text]
            return []
        except Exception:
            return [session.vietnamese_text] if session.vietnamese_text else []
